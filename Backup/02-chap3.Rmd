...
<!--   SOC_needs =c("soc_1", "soc_2"), -->
<!--   SOC_belonging =c("soc_3", "soc_4"), -->
<!--   SOC_influence =c("soc_5", "soc_6"), -->
<!--   SOC_emotion =c("soc_7", "soc_8") -->
<!-- ) -->
<!-- ``` -->
...
<!-- #saveRDS(only_gui, ("data/only_gui_june18_scales.rds")) -->

<!-- ``` -->

```{r include=FALSE}
only_gui <- readRDS(here::here("data/only_gui_june18_scales.rds"))
```
...
score1 <- select(only_gui, "soc_1", "soc_2")

score2 <- select(only_gui, "soc_3", "soc_4")

score3 <- select(only_gui, "soc_5", "soc_6")

score4 <- select(only_gui, "soc_7", "soc_8")
```
...
- Conducting Directional plots with three components
...
fviz_dend(hierarchical_clusters, k = 5, cex = 0.4) # K= the number of clusters in the dendogram
...
### K-mean clustering
```{r echo=FALSE}
kmeans_clusters <- kmeans(cluster_data, centers = 5, nstart = 50) # selecting 4 cluters
...
```{r include=FALSE}
saveRDS(gui_cluster_data, ("data/only_gui_cluster5_Oct12.rds"))
gui_cluster_data <- readRDS(here::here("data/only_gui_cluster5_Oct12.rds"))
```
...
```{r include=FALSE}
# SOC Breakdown
res.aov <- aov(SOC_needs ~ clusters, data = gui_cluster_data)
summary(res.aov) # not Significant

## Belonging
res.aov <- aov(SOC_belonging ~ clusters, data = gui_cluster_data)
summary(res.aov) #Significant

pairwise.t.test(
  x = gui_cluster_data$SOC_belonging, # outcome variable
  g = gui_cluster_data$clusters, # grouping variable
  p.adjust.method = "none" # no correction used
) # 1 and 2 are different

## Influence
res.aov <- aov(SOC_influence ~ clusters, data = gui_cluster_data)
summary(res.aov) #Not Significant

pairwise.t.test(
  x = gui_cluster_data$SOC_influence, # outcome variable
  g = gui_cluster_data$clusters, # grouping variable
  p.adjust.method = "none" # no correction used
) # 1 and 2 are different

## Emotions
res.aov <- aov(SOC_emotion ~ clusters, data = gui_cluster_data)
summary(res.aov) #Not Significant

pairwise.t.test(
  x = gui_cluster_data$SOC_emotion, # outcome variable
  g = gui_cluster_data$clusters, # grouping variable
  p.adjust.method = "none" # no correction used
) # 1 and 2 are on the boarder line p=0.051
...
cluster_wise <- gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SoCoh, SOC, #IMI, 
         IM, PComp, PChoice, CNS, Self_Est, Self_Eff, clusters) %>%
...
group5 <- round(cluster_wise$`5`, 2)
group5_rev <- group5 %>%
  select(median) %>%
  t() %>%
  data.frame()

median_clusters <- rbind(group1_rev, group2_rev, group3_rev, group4_rev, group5_rev) %>% select(clusters., everything())
...
median_clusters %>%
  mutate(
    male = c(5, 3, 2, 1, 17),
    female = c(14, 4, 11, 1, 45),
    members_n = c(19, 7, 13, 2, 63)
  ) %>%
...
```{r include=FALSE}
# t test and Effect Size (Gender) for IMI and SOC
#gui_cluster_data <- readRDS(here::here("data/only_gui_cluster_June18.rds"))
old_new <- gui_cluster_data %>%
  filter(clusters == 1 | clusters == 5)
library(agricolae)
...
Median.test(old_new$SOC_needs, old_new$clusters) # PChoice is not significant. p=0.35
Median.test(old_new$SOC_belonging, old_new$clusters) # PChoice is not significant. p=0.02
Median.test(old_new$SOC_influence, old_new$clusters) # PChoice is not significant. p=0.003
Median.test(old_new$SOC_emotion, old_new$clusters) # PChoice is not significant. p=0.014
...
c1 <- old_new %>%
  ...
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Connection to Narure", x = "New VS Long-term Members") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))
...
c2 <- old_new %>%
  ...
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Sense of Community", x = "New VS Long-term Members") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))
...
c3 <- old_new %>%
  ...
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Social Cohesion", x = "New VS Long-term Members") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))
...
```{r echo=FALSE}
g1 <- old_new %>%
  select(commitment_coded, clusters) %>%
  ggplot(aes(x = clusters, y = commitment_coded, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Duration of Commitment", x = "") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))


g2 <- old_new %>%
  select(frequency_coded, clusters) %>%
  ggplot(aes(x = clusters, y = frequency_coded, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Frequency of Visits", x = "") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))

g3 <- old_new %>%
  select(number_programs, clusters) %>%
  ggplot(aes(x = clusters, y = number_programs, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Number of Programs Attended", x = "") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))
```
...
```{r}
old_new %>%
  select(age, number_programs, commitment_coded, frequency_coded, number_programs, clusters) %>%
  mutate(clusters = case_when(
    clusters == 1 ~ "New Members",
    TRUE ~ "Long-term Members"
  )) %>%
  group_by(clusters) %>%
  summarize_all(median) %>%
  set_colnames(c("Group", "Age", "Number of Program Types Attended", "Duration of Commitment (Months)", "Frequency of Visits (per year)")) %>%
  gt() %>%
  fmt_number(
    columns = vars("Age", "Number of Program Types Attended", "Duration of Commitment (Months)", "Frequency of Visits (per year)"),
    decimals = 1
  )
```
...
```{r}
old_new %>%
  select(number_programs, commitment_coded, frequency_coded, number_programs, clusters) %>%
  mutate(clusters = case_when(
    clusters == 1 ~ "New Members",
    TRUE ~ "Long-term Members"
  )) %>%
  group_by(clusters) %>%
  summarize_all(median) %>%
  set_colnames(c("Group", "Number of Program Types Attended", "Duration of Commitment (Months)", "Frequency of Visits (per year)")) %>%
  gt() %>%
  fmt_number(
    columns = vars("Number of Program Types Attended", "Duration of Commitment (Months)", "Frequency of Visits (per year)"),
    decimals = 1
  )
```
...
```{r}
s1 <- old_new %>%
  select(SOC_needs, clusters) %>%
  ggplot(aes(x = clusters, y = SOC_needs, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Needs Fulfillment", x = "") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))

s2 <- old_new %>%
  select(SOC_belonging, clusters) %>%
  ggplot(aes(x = clusters, y = SOC_belonging, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Belonging", x = "") +
...
s3 <- old_new %>%
  select(SOC_influence, clusters) %>%
  ggplot(aes(x = clusters, y = SOC_influence, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Influence", x = "") +
...
s4 <- old_new %>%
  select(SOC_emotion, clusters) %>%
  ggplot(aes(x = clusters, y = SOC_emotion, group = clusters, fill = clusters)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(y = "Emotional Connection", x = "") +
  scale_fill_manual(values = c("#DDE5D5", "#629952"))
...
library(lsr)
gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IM, PComp, PChoice, Self_Est, Self_Eff, CNS, RC1, RC2, clusters, SOC_needs, SOC_belonging, SOC_influence, SOC_emotion) %>%
...
cor_test_results <- gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SOC, SOC_needs, SOC_belonging, SOC_influence, SOC_emotion, SoCoh, IM, PComp, PChoice, Self_Est, Self_Eff, CNS, RC1, RC2, clusters) %>%
  rename("Frequency" = frequency_coded, "Commitment" = commitment_coded, "# of Programs" = number_programs) %>%
  mutate(clusters = as.numeric(clusters)) %>%
...
# SOC
full.model <- lm(
  formula = SOC ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)

step(
  object = full.model, # start at the full model
  direction = "backward" # allow it remove predictors but not add them
)

# CNS
full.model <- lm(
  formula = CNS ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# IMI
full.model <- lm(
  formula = IMI ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# IM
full.model <- lm(
  formula = IM ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# PComp
full.model <- lm(
  formula = PComp ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# PChoice
full.model <- lm(
  formula = PChoice ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# Self_Est
full.model <- lm(
  formula = Self_Est ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# Self_Eff
full.model <- lm(
  formula = Self_Eff ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# SoCoh
full.model <- lm(
  formula = SoCoh ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
# Influence
full.model <- lm(
  formula = SOC_influence ~ age + Frequency + Commitment + `# of Programs` + clusters + RC1 + RC2,
  data = model_data
)
...
```{r include=FALSE}
model_gui_2 <- lm(SOC_belonging ~ `# of Programs`, data = model_data)
...
```{r results="hide"}
model_gui_6 <- lm(
  formula = IM ~ age + Commitment + `# of Programs` + clusters,
  data = model_data
)
...
# CNS
full.model <- lm(
  formula = CNS ~ age + Frequency + Commitment + `# of Programs` + SOC + SoCoh + Self_Est + Self_Eff + clusters + RC1 + RC2 + IM + PComp + PChoice,
  data = model_data
)
step(
  object = full.model, # start at the full model
...
# IM
full.model <- lm(
  formula = IM ~ SoCoh + Frequency + Commitment + `# of Programs` + SOC + Self_Est + Self_Eff + clusters + RC1 + RC2 + PComp + PChoice,
  data = model_data
)
step(
  object = full.model, # start at the full model
...
# EFF
full.model <- lm(
  formula = Self_Eff ~ Frequency + Commitment + `# of Programs` + SOC + SoCoh + Self_Est + Self_Eff + clusters + RC1 + RC2 + PComp + PChoice + IM,
  data = model_data
)
step(
  object = full.model, # start at the full model
...
# NUmber of Programs
full.model <- lm(
  formula = `# of Programs` ~ age + Frequency + Commitment + SOC + SoCoh + Self_Est + Self_Eff + PComp + PChoice + IM,
  data = model_data
)
step(
  object = full.model, # start at the full model
...
```{r echo=FALSE}
tibble(
  "Variables" = c("Nature Connection", "Self Esteem", "Self Efficacy"),
  "Median of Non GUI" = c(5.1, 4.75, 5.25),
  "Median of Long-term GUI" = c(5.5, 5.25, 5.36),
  "p Value" = c(0.14, 0.22, 0.76)
) %>%
  gt() %>%
  tab_header(
