...
```{r echo=FALSE}
#Showing the group wise data
library(reshape)

#Showing 4 graphs by clusters
#x is rating 1 to 7, and y is scales

gg1 <- gui_mar20_scales %>%
  filter(gui_non_gui == "yes") %>% 
  select(SoCoh, SOC, CNS, Self_Est, Self_Eff) %>%
  melt() %>% 
  ...
  labs(x="Scales", y="Rating (1 to 7)", title="GUI Members (n=104)")+
  #coord_flip()+
  theme_minimal() +
  theme(legend.position = "none")+
  theme(plot.title = element_text(face="bold", size = 12, hjust = 0.5))

gg2 <- gui_mar20_scales %>%
  filter(gui_non_gui == "never") %>% 
  select(SoCoh, SOC, CNS, Self_Est, Self_Eff) %>%
  melt() %>% 
  ggplot(aes(factor(variable), value))+
  geom_boxplot(aes(fill=variable, alpha=0.8))+
  #geom_point(position = position_jitter(w = 0.2, h = 0), alpha=0.4)+
  labs(x="Scales", y="Rating (1 to 7)", title="Non-GUI Members (n=89)")+
  #coord_flip()+
  theme_minimal() +
  theme(legend.position = "none")+
  theme(plot.title = element_text(face="bold", size = 12, hjust = 0.5))
