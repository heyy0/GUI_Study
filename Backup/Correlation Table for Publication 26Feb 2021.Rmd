...
- __`Correlation Test (Spearman's Rank Test with Holm adjustment)`__ shows correlation r by the figures and highlights: towards __shades of blue__ mean __negative__ and __shades of red__ mean __positive correlations__. Those squares with no highlights (white) are non-significant correlations (p> 0.05).
...
```{r include=FALSE}
gui_cluster_data %>%
  dplyr::select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IMI, IM, PComp, PChoice, Self_Est, Self_Eff, CNS, RC1, RC2, clusters) %>% rename("Frequency" = frequency_coded,"Commitment" = commitment_coded, "# of Programs" = number_programs) %>% mutate(clusters=as.numeric(clusters)) %>% pairs()
```
...
```{r}
upper %>% mutate(Variables = c("age", "Frequency", "Commitment",  "# of Programs", "SOC", "SoCoh", "IMI", "IM", "PComp", "PChoice", "Self_Est", "Self_Eff")) %>% 
