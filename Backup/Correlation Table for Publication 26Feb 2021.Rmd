...
- Those squares with no stars (white) are non-significant correlations (p> 0.05). * = _p_ < 0.05, "* * *" = _p_ < 0.001, "* * * * " = _p_ < 0.001
...
table_1 <- gui_cluster_data %>%
  dplyr::select(
    age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, CNS, IMI, IM, PComp, PChoice, Self_Est, Self_Eff # , CNS, RC1, RC2, clusters
  ) %>%
  rename("Frequency" = frequency_coded, "Commitment" = commitment_coded, "# of Programs" = number_programs) %>%
  correlate(test = TRUE, corr.method = "spearman", p.adjust.method = "holm")
...
```{r}
upper %>%
  mutate(Variables = c("age", "Frequency", "Commitment", "# of Programs", "SOC", "SoCoh", "CNS", "IMI", "IM", "PComp", "PChoice", "Self_Est", "Self_Eff")) %>%
