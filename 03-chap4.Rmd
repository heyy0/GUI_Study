...
ggplot(melted_only_cor, aes(Var2, Var1, fill = cor_r)) +
  ...
  labs(title = "Fig 1. Correlation Test (Spearman)") +
...
ggplot(melted_only_cor, aes(Var2, Var1, fill = pvalue)) +
  ...
  scale_fill_gradient2(
    low = "red", high = "white",
    midpoint = 0.05, limit = c(0, 0.2), space = "Lab",
    name = "p-value"
  ) +
  theme_minimal() +
  theme(
  ...
  labs(title = "Fig 2. Correlation Test highlighted with p-value (Spearman)") +
  geom_text(aes(Var2, Var1, label = cor_r), color = "black", size = 5) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
...
pp_2 <- pp_1 %>% # Cronbacks Alpha
  mutate(
    ...
    test_statistics = c("p < .01", "p < .01"), # test statistics
...
b1 <- data_pre_post_scales %>%
  select(ROS, pre_post) %>%
  ggplot(aes(x = pre_post, y = ROS, fill = pre_post)) +
  geom_boxplot() +
  theme(legend.position = "None") +
  labs(x = "Post VS Pre", y = "ROS")
b2 <- data_pre_post_scales %>%
  select(PANAS, pre_post) %>%
  ggplot(aes(x = pre_post, y = PANAS, fill = pre_post)) +
  geom_boxplot() +
  theme(legend.position = "None") +
  labs(x = "Post VS Pre", y = "PANAS")
```
