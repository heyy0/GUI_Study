...
```{r include=FALSE}
tbl_1 <- tibble(vars = c(1, 2, 3, 4, 5, 6), scales = c("Social Cohesion", "Sense of Community", "Connection to Nature", "Self Esteem", "Self Efficacy", "Intrinsic Motivation"), Cronbachs_Alpha = c(0.76, 0.92, 0.89, 0.76, 0.91, 0.78))
```
...
```{r include=FALSE}
# Saving Data Scales
#saveRDS(only_gui_scales, ("data/only_gui_mar20_scales.rds"))
#only_gui_scales <- readRDS("/data/only_gui_mar20_scales.rds") # for coding
only_gui_scales <- readRDS(here::here("data/only_gui_mar20_scales.rds")) #for knitting
```

## Descriptive statistics
```{r include=FALSE}
tbl_2 <- only_gui_scales %>%
  select(age, frequency_coded, commitment_coded, SoCoh, SOC, CNS, IMI, Self_Est, Self_Eff) %>%
  describe()

tbl_2 <- tbl_2 %>%
  mutate(variables = c("Age", "Frequency of Visits (per year)", "Total Duration of Commitment (months)", "Social Cohesion", "Sense of Community", "Connection to Nature", "Intrinsic Motivation", "Self Esteem", "Self Efficacy")) %>%
...
only_gui_scales %>%
  ...
  geom_vline(mapping = aes(xintercept = 15), color = "red", alpha = 0.5) +
  scale_y_continuous(breaks = c(1, 5, 20, 100, 200, 300)) +
  scale_x_continuous(breaks = c(0, 5, 15, 36, 50, 100, 136)) +
  labs(y = "Frequency of Visits per Month", x = "Total duration of commitment (months)", title = "Frequency and Commitment", subtitle = "*Red lines are median for frequency of visits and commitment (5 and 15, respectively)")
```
...
program_count %>%
  ...
  arrange(count) %>%
  ggplot() +
  geom_col(aes(x = programs, y = count), fill = "darkgreen", alpha = 0.8) +
  coord_flip() +
...
```{r include=FALSE}
only_cor <- only_gui_scales %>%
  select(frequency_coded, commitment_coded, program_percent) %>%
  correlate(test = TRUE, corr.method = "pearson", p.adjust.method = "none")
```

```{r include=FALSE}
pre_pvalue <- round(only_cor$p.value, 2)
pre_cor <- round(only_cor$correlation, 2)
...
melted_cor_r <- melt(lower_tri, na.rm = TRUE) %>% rename(cor_r = value)
melted_cor_p <- melt(lower_tri_p, na.rm = TRUE) %>%
  rename(pvalue = value) %>%
  select(pvalue)
...
ggplot(melted_only_cor, aes(Var2, Var1, fill = cor_r)) +
  ...
  labs(title = "Correlation Test (Pearson)") +
  geom_text(aes(Var2, Var1, label = cor_r), color = "black", size = 5) +
  theme(
...
ggplot(melted_only_cor, aes(Var2, Var1, fill = pvalue)) +
  ...
  labs(title = "Correlation Test (Pearson)") +
  geom_text(aes(Var2, Var1, label = pvalue), color = "black", size = 5) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    panel.grid.major = element_blank(),
...
tbl_normality_2 <- tibble(
  variables = c("Age", "Frequency of Visits (per year)", "Total Duration of Commitment (months)", "Social Cohesion", "Sense of Community", "Connection to Nature", "Intrinsic Motivation", "Self Esteem", "Self Efficacy"),
  shapiro_test = c("p <.001", "p <.001", "p <.001", "p <.001", "0.096*", "p <.001", "0.118*", "p <.001", "p <.001")
)
```
...
# Summary of Table
tbl_gender <- tibble(
  vars = c(1, 2, 3, 4, 5, 6, 7, 8, 9),
  scales = c("Age", "Frequency", "Commitment", "Social Cohesion", "Sense of Community", "Connection to Nature", "Intrinsic Motivation", "Self Esteem", "Self Efficacy"),
  male_median = c(35.5, 9, 10, 6.0, 5.125, 5.20, 5.50, 4.50, 5.00),
  female_median = c(36.0, 4, 16, 6.2, 5.25, 5.70, 5.87, 5.25, 5.5),
  Median_test_p_value = c("0.64", "0.079", "0.30", "0.40", "0.40", "0.82", "0.25", "0.09", "0.041*")
)
```
  
As the difference in the sample size is large (75 female and 28 male), the median test was conducted to see if there are any significant differences between female and male members in the respective variables. Although the difference in the frequency of visits is large (male members visit GUI more than twice as often as female members do), no significant difference was observed. __Self-efficacy__ is the only variable that shows a significant difference: __female members have a higher sense of self-efficacy__.
...
# saveRDS(gui_cluster_data, ("4_Survey_Experiment/Data/only_gui_cluster_apr16.rds"))
gui_cluster_data <- readRDS(here::here("/data/only_gui_cluster_apr16.rds"))
```
...
cluster_wise <- gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SoCoh, SOC, IMI, CNS, Self_Est, Self_Eff, clusters) %>%
...
```{r results="hide"}
Median.test(old_new$SOC, old_new$clusters) # Sense of Community is significant
...
- It's observed that __Sense of Community (`SOC`),  Social Cohesion (`SoCoh`), and Intrinsic Motivation (`IMI`)__ show __significant correlations (moderate to strong)__ among them.   

- Frequency (`frequency_coded`) and commitment (`commitment_coded`) also show significant correlations (moderate, r=0.5, Spearman). However, as discussed earlier, __the correlation test with Pearson__ only showed a weak correlation (r=0.23, Pearson).
  
- __1) those who visit GUI frequently__ and __2) those who commit GUI in the long duration__ have __a higher sense of community__ and __a higher Intrinsic Motivation__. Earlier studies from Community Psychology support these findings: __sense of community and intrinsic motivation can be built through frequent interactions over the long term__ (Christens & Peterson, 2010; Bidee, 2013; Oostlander et al., 2013; ).  
  
- There is __a strong correlation between Nature Connection and Intrinsic motivation (r=0.465)__, implying that __those who are self-motivated also have a high sense of nature connection.__
  
- The number of __program types attended__ (`number_programs`) shows __significant positive correlations__ with __frequency, commitment, Sense of Community (`SOC`),  Social Cohesion (`SoCoh`), and Intrinsic Motivation (`IMI`)__. Hence, __more committed members__ with __a higher sense of community__ engage in __a variety of programs__. It is also interesting to note that `number_programs` shows a significant negative correlation with `age`: __older GUI members__ attend __less variety of GUI programs__.

- This correlation test did not show significant correlations between __1) frequency and commitment, and 2) Social Cohesion and Nature connection__. This suggests that __Social Cohesion and Nature Connection can be built among GUI members regardless of their duration of commitment and frequency of visits.__ As the focus group discussion with GUI volunteers noted, perhaps GUI members can gain Social Cohesion and Nature Connection immediately through their engagement at GUI. 
...
library(lsr)
gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IMI, Self_Est, Self_Eff, CNS, RC1, RC2, clusters) %>% rename("Frequency" = frequency_coded,"Commitment" = commitment_coded, "# of Programs" = number_programs) %>% mutate(clusters=as.numeric(clusters)) %>% pairs()
```


<!-- ```{r include=FALSE} -->
<!-- only_cor <- only_gui_scales %>% -->
<!--   select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IMI, Self_Est, Self_Eff, CNS) %>% -->
...
<!--     axis.text.y = element_text(size = 8) -->
<!--   ) + -->
<!--   labs(title = "Fig 2.Correlation Test with p-value (Spearman)") + -->
<!--   geom_text(aes(Var2, Var1, label = cor_r), color = "black", size = 3) + -->
<!--   theme( -->
<!--     axis.title.x = element_blank(), -->
<!--     axis.title.y = element_blank(), -->
...
cor_test_results <- gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IMI, Self_Est, Self_Eff, CNS, RC1, RC2, clusters) %>% rename("Frequency" = frequency_coded,"Commitment" = commitment_coded, "# of Programs" = number_programs) %>% mutate(clusters=as.numeric(clusters)) %>% 
  correlate(test = TRUE, corr.method = "spearman", p.adjust.method = "none")

cor_samples %>% cor(method="spearman")
...
model_data <- gui_cluster_data %>%
  select(age, frequency_coded, commitment_coded, number_programs, SOC, SoCoh, IMI, Self_Est, Self_Eff, CNS, RC1, RC2, clusters) %>% rename("Frequency" = frequency_coded,"Commitment" = commitment_coded, "# of Programs" = number_programs)
```
...
#CNS
full.model <- lm(formula = CNS ~  age + Frequency +Commitment +`# of Programs`,  
                   data = model_data  
 )
...
#IMI
full.model <- lm(formula = IMI ~  age + Frequency +Commitment +`# of Programs`,  
                   data = model_data  
 )
...
#Self_Est
full.model <- lm(formula = Self_Est ~  age + Frequency +Commitment +`# of Programs`,  
                   data = model_data  
 )
...
#Self_Eff
full.model <- lm(formula = Self_Eff ~  age + Frequency +Commitment +`# of Programs`,  
                   data = model_data  
 )
...
#SoCoh
full.model <- lm(formula = SoCoh ~  age + Frequency +Commitment +`# of Programs`,  
                   data = model_data  
 )
...
```{r results="hide"}
model_gui_1 <- lm(SOC ~ Frequency + `# of Programs`+clusters, data = model_data %>% mutate(clusters=factor(clusters, levels = c("3", "1", "2", "4"))))
summary(model_gui_1) # R-squared =0.11. Number of programs is Significant
```
...
------------- | ------------- | -------------|-------------|
Sense of Community  | 0.12 | `# of Programs` | NA|
Nature Connection  | Not significant  | NA | NA |
Intrinsic Motivation | 0.10  | `# of Programs` | NA |
...
```{r include=FALSE}
#Self_Eff
full.model <- lm(formula = CNS ~  age + Frequency +Commitment +`# of Programs`+IMI+SOC+SoCoh+Self_Est+Self_Eff,  
                   data = model_data  
 )
...
```{r results="hide"}
model_gui_6 <- lm(CNS ~ Commitment + IMI + SOC + Self_Eff, data = model_data)
summary(model_gui_6)
# Significant model
```
...
```{r results="hide"}
model_gui_3 <- lm(CNS ~ IMI, data = only_gui_scales)
glance(model_gui_3)
summary(model_gui_3)
# Significant model
```
