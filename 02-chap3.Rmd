...
<!-- only_gui_scales <- cbind(only_gui_scales, dfscores) -->
<!-- ``` -->
...
model_gui_7 <- lm(formula = PComp ~ `# of Programs`, 
    data = model_data)
summary(model_gui_7) # R-squared is 0.164. Number of programs and cluster 2 are significant
```

```{r results="hide"}
model_gui_7 <- lm(formula = PChoice ~ `# of Programs`, 
    data = model_data)
summary(model_gui_7) # R-squared is 0.164. Number of programs and cluster 2 are significant
```
...
#IMI
full.model <- lm(formula = IM ~  age + Frequency +Commitment +`# of Programs`+SOC+SoCoh+Self_Est+Self_Eff+clusters+RC1+RC2+PComp+PChoice,  
                   data = model_data  
 )
...
#EFF
full.model <- lm(formula = Self_Eff ~  age + Frequency +Commitment +`# of Programs`+SOC+SoCoh+Self_Est+Self_Eff+clusters+RC1+RC2+PComp+PChoice+IM,  
                   data = model_data  
 )
step( object = full.model,     # start at the full model
       direction = "backward"   # allow it remove predictors but not add them
