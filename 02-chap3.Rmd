...
```{r include=FALSE}
#CNS
full.model <- lm(formula = CNS ~  age + Frequency +Commitment +`# of Programs`+IMI+SOC+SoCoh+Self_Est+Self_Eff+clusters+RC1+RC2,  
                   data = model_data  
 )
...
#IMI
full.model <- lm(formula = IMI ~  age + Frequency +Commitment +`# of Programs`+CNS+SOC+SoCoh+Self_Est+Self_Eff+clusters+RC1+RC2,  
                   data = model_data  
 )
...
#EFF
full.model <- lm(formula = Self_Eff ~  age + Frequency +Commitment +`# of Programs`+CNS+SOC+SoCoh+Self_Est+IMI+clusters+RC1+RC2,  
                   data = model_data  
 )
...
```{r results="hide"}
model_gui_6 <- lm(IMI ~ SOC + SoCoh + Self_Eff, data = model_data)
summary(model_gui_6)
# Significant model
```

```{r results="hide"}
model_gui_3 <- lm(CNS ~ Commitment + IMI + SOC + Self_Eff, data = model_data)
summary(model_gui_3)
# Significant model
```
...
```{r results="hide"}
model_gui_4 <- lm(Self_Eff ~ CNS + SOC + Self_Est + IMI, data = model_data)
summary(model_gui_4)
# R-squared = 0.4893. All coefficients are significant. How to interpret this?
```
